...
(function () {
  ...
  const sysById = (id) => D.systems.find(s => s.id === id);

  /* ---------- state ---------- */
  const state = { reviews: {}, cases: {}, activity: D.activity.slice() };
  try {
    const saved = JSON.parse(sessionStorage.getItem('nigel-proto') || 'null');
    if (saved && saved.v === 2) Object.assign(state, saved.state);
  } catch (e) { /* storage unavailable, run in memory */ }
  function persist() {
    try { sessionStorage.setItem('nigel-proto', JSON.stringify({ v: 2, state: { reviews: state.reviews, cases: state.cases, activity: state.activity } })); } catch (e) { /* ignore */ }
  }
  ...
  function logActivity(what, system) { state.activity.unshift({ when: 'Just now', what, system: system || 'nigel' }); persist(); }

  /* ---------- clock ---------- */
  function tickClock() {
    const now = new Date();
    const days = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'], months = ['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC'];
    $('#clock-date').textContent = `${days[now.getDay()]} ${months[now.getMonth()]} ${now.getDate()}, ${now.getFullYear()}`;
    let h = now.getHours(); const ampm = h >= 12 ? 'PM' : 'AM'; h = h % 12 || 12;
    $('#clock-time').textContent = `${String(h).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')} ${ampm}`;
  }
  ...
  function refreshLabels() {
    for (const s of D.systems) {
      const el = labelsRoot.querySelector(`[data-system="${s.id}"]`); if (!el) continue;
      const n = openReviews(s.id).length;
      el.classList.toggle('is-amber', n > 0);
      $('.label-sub', el).textContent = n > 0 ? `${n} need${n === 1 ? 's' : ''} attention` : '';
      SC.setAttention(s.id, n > 0);
    }
    const total = openReviews().length;
    const badge = $('#attention-badge'); badge.textContent = total; badge.classList.toggle('is-zero', total === 0);
  ...
  function parse() {
    ...
    let view = parts[0] || 'starcloud';
    if (view === 'attention') view = 'today';
    if (view === 'records') view = 'office';
    if (view === 'systems') {
  ...
  const VIEWS = ['starcloud', 'today', 'office', 'settings', 'activity', 'systems', 'sectors', 'records', 'record'];
  function render() {
    const r = parse();
    const sys = r.sid && sysById(r.sid);
    const sec = sys && r.sec && sys.sectors.find(x => x.id === r.sec);
    const hh = r.rid && D.households[r.rid];
    if (!VIEWS.includes(r.view) || (r.sid && !sys)) { go('/'); return; }
    ...
    for (const b of document.querySelectorAll('.nav-item')) {
      ...
      b.classList.toggle('is-active', p === '/' ? here === '/' : here === p || here.startsWith(p + '/'));
    }
    ...
    const body = ({
      starcloud: () => '', today: viewToday, office: viewOffice, settings: viewSettings, activity: viewActivity, systems: viewSystems,
      sectors: () => viewSectors(sys), records: () => viewRecords(sys, sec), record: () => viewRecord(sys, sec, hh)
    })[r.view]();
    stage.innerHTML = (r.view === 'starcloud' ? '' : renderCrumbs(r, sys, sec, hh)) + body;
    bindStage();
  }

  function renderCrumbs(r, sys, sec, hh) {
    const items = [['StarCloud', '/']];
    if (r.view === 'today') items.push(['Today', '/today']);
    if (r.view === 'office') items.push(['My Office', '/office']);
    if (r.view === 'settings') items.push(['Settings', '/settings']);
    if (r.view === 'activity') items.push(['Activity', '/activity']);
    if (r.view === 'systems') items.push(['Systems', '/systems']);
    ...
    return `<nav class="crumbs" aria-label="Breadcrumb">${items.map(([name, path], i) => {
      const last = i === items.length - 1, back = i === items.length - 2;
      return `${i ? '<span class="crumb-sep">›</span>' : ''}<button class="crumb${last ? ' is-current' : ''}${back ? ' crumb-back' : ''}" data-go="${path}">${back ? '‹ ' : ''}${esc(name)}</button>`;
    }).join('')}</nav>`;
  }
  ...
  function viewSectors(sys) {
    ...
    return `<section class="panel"><div class="panel-head"><div><h1><small>System</small>${esc(sys.name)}</h1><p class="lede">${esc(sys.tagline)}. ${n ? `<span style="color:var(--amber)">${n} review${n === 1 ? '' : 's'} open.</span>` : 'Nothing needs attention here.'}</p></div>
      ${n ? `<button class="btn btn-amber" data-go="/today">Open today</button>` : ''}</div>
    <div class="panel-body"><div class="grid">${sys.sectors.map(sec => {
      ...
      const target = sec.attention ? '/today' : hasRecords ? `/systems/${sys.id}/${sec.id}` : '';
  ...
  function viewOffice() {
    const rows = Object.values(D.households).map(h => {
      const c = caseOf(h.id);
      return `<button class="row" data-go="/systems/paraclete/pending/${h.id}"><span><span class="row-title">${esc(h.name)}</span><span class="row-sub">Paraclete 1 · ${esc(h.caseTitle)} · ${money(h.premium)}</span></span><span class="row-meta"><span class="stage-tag${c.stage >= 3 ? ' is-live' : ''}">${STAGES[c.stage]}</span></span></button>`;
    }).join('');
    return `<section class="panel"><div class="panel-head"><div><h1><small>My Office</small>Open cases</h1><p class="lede">Households with work in progress. In this prototype every open case lives under Paraclete 1 → Pending annuity clients.</p></div><button class="btn btn-ghost" data-go="/activity">Activity</button></div><div class="panel-body"><div class="list">${rows}</div></div></section>`;
  }

  function viewSettings() {
    return `<section class="panel"><div class="panel-head"><div><h1><small>NIGEL</small>Settings</h1></div></div><div class="placeholder">Preferences, connections and voice are not part of this prototype.<br>Nothing here is connected to a live system.</div></section>`;
  }
  ...
  function viewToday() {
    ...
    for (const r of D.reviews) {
      const key = r.system + '/' + r.sector;
      let g = groups.find(x => x.key === key);
      if (!g) { const s = sysById(r.system); g = { key, name: s.sectors.find(x => x.id === r.sector).name, parent: s.name, list: [] }; groups.push(g); }
      g.list.push(r);
    }
    return `<section class="panel"><div class="panel-head"><div><h1><small>Today · ${esc($('#clock-date').textContent)}</small>${total ? `${total} item${total === 1 ? '' : 's'} need${total === 1 ? 's' : ''} attention` : 'All clear'}</h1><p class="lede">${total ? 'Each action below is recorded in Activity. Nothing is sent anywhere.' : 'Everything is resolved. New items appear here as NIGEL finds them.'}</p></div>
    ...
    <div class="panel-body">${groups.map(g => {
      const n = g.list.filter(r => reviewStatus(r) === 'open').length;
      return `<div class="group-title"><span>${esc(g.name)}</span><span class="n">${n} open</span><span class="parent">${esc(g.parent)}</span></div><div class="list">${g.list.map(reviewCard).join('')}</div>`;
    }).join('')}
    <div class="group-title" style="margin-top:26px"><span>Recent activity</span></div><div class="timeline">${state.activity.slice(0, 6).map(activityRow).join('')}</div>
    </div></section>`;
  }
  function reviewCard(r) {
    const st = state.reviews[r.id]; const done = st && st.status !== 'open';
  ...
  const activityRow = (a) => `<div class="tl${a.when === 'Just now' ? ' is-new' : ''}"><span class="w">${esc(a.when)}</span><span class="x">${esc(a.what)}</span></div>`;
  function viewActivity() {
    ...
    <div class="panel-body"><div class="timeline">${state.activity.map(activityRow).join('')}</div></div></section>`;
  }
  ...
  function viewRecord(sys, sec, h) {
    ...
    const docs = h.documents.map(d => {
      ...
      return `<div class="doc"><span>${esc(d.name)}</span><span class="st${/needs|not started/i.test(status) ? ' is-amber' : ''}">${esc(status)}</span></div>`;
    }).join('');
    ...
    <div class="panel-body"><div class="two-col">
      <div class="card"><h3>Household</h3>
        ...
        <h3 style="margin-top:20px">Documents</h3><div class="docs">${docs}</div>
      </div>
      <div class="card"><h3>Case workflow</h3>
        ...
        <h3 style="margin-top:20px">Timeline</h3><div class="timeline">${timeline}</div>
      </div>
    </div></div></section>`;
  }
  ...
  /* ---------- actions ---------- */
  function bindStage() {
    for (const el of document.querySelectorAll('#stage [data-go]')) el.addEventListener('click', () => go(el.dataset.go));
    for (const el of document.querySelectorAll('#stage [data-act]')) el.addEventListener('click', () => action(el.dataset));
  }
  function action(d) {
    const id = d.id;
    if (d.act === 'review') {
      const r = D.reviews.find(x => x.id === id);
      const text = { approve: 'Approved. NIGEL will carry out the proposed action (simulated).', changes: 'Changes requested. NIGEL will revise the proposal (simulated).', defer: 'Deferred one week. It will return to Today (simulated).' }[d.outcome];
  ...
  function draftMemo(id) {
    ...
    const tick = () => {
      ...
      i = Math.min(full.length, i + 6); c.memo = full.slice(0, i);
      const m = document.getElementById('memo-' + id); if (m) m.textContent = c.memo;
  ...
  function confirmSubmit(id) {
    ...
    modal('Submit to carrier?', `<p>This is a <strong>simulated</strong> submission for <strong>${esc(h.name)}</strong>. The prototype will animate a transmission and produce a fictional reference number. Nothing is sent to ${esc(h.proposedCarrier)} or anywhere else.</p>`, [
      ['Cancel', 'btn btn-ghost', null], ['Simulate submission', 'btn btn-primary', () => runSubmit(id)]
    ]);
  }
  function runSubmit(id) {
    const h = D.households[id], c = caseOf(id);
    c.submission = { step: 0, done: false }; render();
    const steps = [900, 1400, 900]; let s = 0;
    const next = () => {
  ...
  function modal(title, body, actions) {
    ...
    for (const [label, cls, fn] of actions) {
      ...
      b.addEventListener('click', () => { m.hidden = true; if (fn) fn(); }); a.appendChild(b);
    }
  ...
  function say(role, html, chips) {
    ...
    log.appendChild(m); log.hidden = false; log.scrollTop = log.scrollHeight; return m;
  }
  const ALIASES = { aum: 'investments', insurance: 'servicing', annuity: 'paraclete', annuities: 'paraclete', 'god pod': 'godpod' };
  function reply(q) {
    const t = q.toLowerCase();
    const open = openReviews();
    let sysByName = D.systems.find(s => t.includes(s.name.toLowerCase()) || t.includes(s.id));
    if (!sysByName) { const k = Object.keys(ALIASES).find(a => t.includes(a)); if (k) sysByName = sysById(ALIASES[k]); }
    if (/attention|review|needs|what.*(do|urgent)|flag|today/.test(t)) {
      if (!open.length) return ['Nothing needs attention right now. All sample reviews are resolved.', [['Open today', '/today']]];
      return [`${open.length} item${open.length === 1 ? '' : 's'} need attention: ` + open.map(r => `<strong>${esc(r.title)}</strong> (${sysById(r.system).sectors.find(x => x.id === r.sector).name}, due ${r.due})`).join('; ') + '.', [['Open today', '/today'], [`Go to ${sysById(open[0].system).name}`, `/systems/${open[0].system}`]]];
    }
    ...
    if (/pending|annuity client|paraclete/.test(t) && !/open|go|show/.test(t)) return ['Four households are pending in Paraclete 1: Okonkwo-Reyes (suitability), Thornbury (illustration), Halvorsen (intake) and Adeyemi-Park (submitted).', [['Open pending clients', '/systems/paraclete/pending']]];
    ...
    if (/office|cases|records/.test(t)) return ['My Office lists every open case.', [['Open My Office', '/office']]];
    ...
    if (/help|what can|how do/.test(t)) return ['I can navigate the cloud, summarise what needs attention, and walk a pending annuity case through suitability, illustration and a simulated submission. Try “what needs attention”, “open Investments”, or “where is the Okonkwo-Reyes case”.', [['What needs attention', '', () => ask('what needs attention')], ['Open Systems', '/systems']]];
  ...
  function ask(q) {
    ...
    setTimeout(() => {
      th.remove(); const [html, chips, fn] = reply(q); say('nigel', html, chips); if (fn) setTimeout(fn, 250);
    }, 550 + Math.random() * 350);
  }
  $('#convo-form').addEventListener('submit', (e) => { e.preventDefault(); const inp = $('#convo-input'); const q = inp.value.trim(); if (!q) return; inp.value = ''; ask(q); });
  $('#convo-mic').addEventListener('click', () => toast('Voice input is not part of this prototype.'));
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (document.body.classList.contains('menu-open')) { document.body.classList.remove('menu-open'); return; }
    const back = document.querySelector('.crumb-back'); if (back) back.click(); else if (location.hash && location.hash !== '#/') go('/');
  });

  /* ---------- shell wiring ---------- */
  for (const b of document.querySelectorAll('.nav-item')) b.addEventListener('click', () => go(b.dataset.nav));
  $('#topbar-attention').addEventListener('click', () => go('/today'));
