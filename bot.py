...
TELEGRAM_MAX_LEN = 4000          # Telegram hard cap is 4096; leave a buffer
...
def get_calendar_events(days_ahead: int = 1) -> str:
    ...
    - days_ahead=1  → today only, flat bullet list (morning brief default)
    ...
    try:
            ...
            try:
                ...
                for event in events_result.get("items", []):
                    ...
                    if "T" in start_dt_str:
                        ...
                        # All-day event — start_dt_str is like "2026-06-29"
                        dt_naive = datetime.strptime(start_dt_str, "%Y-%m-%d")
                        time_str = "All day"
                        date_str = start_dt_str
                        date_label = dt_naive.strftime("%A, %B %-d")

                    all_events.append((start_dt_str, date_str, date_label, time_str, summary))
        ...
        # ── Single-day view: flat list (keeps morning brief compact) ──────────
        if days_ahead == 1:
            return "\n".join(f"• {ev[3]} — {ev[4]}" for ev in all_events)

        # ── Multi-day view: group by date with emoji headers ──────────────────
        ...
        for _, date_str, date_label, time_str, summary in all_events:
            ...
            if date_str == today_str:
                ...
                label += " (Tomorrow)"
            events_by_date[(date_str, label)].append(f"  • {time_str} — {summary}")

        sections = []
        for date_str, label in sorted(events_by_date.keys(), key=lambda k: k[0]):
            lines = events_by_date[(date_str, label)]
...
async def send_long_message(send_func, text: str, max_len: int = TELEGRAM_MAX_LEN) -> None:
    """
    Split a long text into Telegram-safe chunks and send each one.
    Splits cleanly on newlines so no sentence is cut mid-word.
    """
    if len(text) <= max_len:
...
def _call_claude(messages: list, max_tokens: int = 800) -> str:
...
def build_morning_brief() -> str:
    """Generate today's morning brief using live Calendar (today + 30-day lookahead) and Gmail data."""
    now_et = datetime.now(EASTERN)
    day_str = now_et.strftime("%A, %B %-d")
    weekday = now_et.weekday()  # 0=Mon … 4=Fri

    # Pull live data — today flat list + full 30-day window for context
    calendar_today = get_calendar_events(days_ahead=1)
    calendar_lookahead = get_calendar_events(days_ahead=30)
    ...
    prompt = (
        ...
        f"📅 Today's calendar:\n{calendar_today}\n\n"
        f"📆 30-day calendar (for awareness — surface only what's worth flagging):\n{calendar_lookahead}\n\n"
        ...
        "5. 📋 Reminders — standing day-of-week reminders relevant to PFI operations\n"
        "6. 📆 Coming Up — 1-2 notable events in the next 30 days worth flagging (skip if nothing significant)\n"
        "7. A one-line motivational close\n\n"
        "Format with clear emoji section headers. Keep it tight — under 450 words total. "
        "Lead with what matters most. No fluff."
    )

    return _call_claude([{"role": "user", "content": prompt}], max_tokens=800)
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        "  /calendar — full 30-day calendar view\n"
...
async def cmd_brief(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await send_long_message(update.message.reply_text, brief)
    except Exception as e:
...
async def cmd_calendar(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send a full 30-day calendar view, split into Telegram-safe chunks if needed."""
    if not _is_authorized(update):
        return
    ...
    try:
        ...
        await send_long_message(update.message.reply_text, header + events)
    except Exception as e:
...
async def send_morning_brief(app: Application) -> None:
    ...
    try:
        ...
        brief = build_morning_brief()
        await send_long_message(_send, brief)
